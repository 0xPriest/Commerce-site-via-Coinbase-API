...
{price: 1, name: "values 1" },

{price: 100, name: "values 2" },
{price: 91, name: "values 2" },

{price: 10, name: "values 2" },
  ...
  return (
	...
	<div class="flex flex-col sm:flex-row sm:items-end sm:justify-between mt-6">
		<span class="text-sm font-semibold">1-{options.length} of {options.length}</span>
	...
	<div class="grid 2xl:grid-cols-10 xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-x-6 gap-y-12 w-full mt-6">
...
<CoinbaseCommerceButton checkoutId={'RBPY53LE'}/>
			<a href="#" class="block h-64 rounded-lg shadow-lg bg-white text-center -90">@{thing.name} </a>
			<div class="flex items-center justify-between mt-3">
				<div>
					<a href="#" class="font-medium">@{thing.name}</a>
					<a class="flex items-center" href="#">
						<span class="text-xs font-medium text-gray-600">by</span>
                
						<span class="text-xs font-medium ml-1 text-indigo-500">...</span>
					</a>
				</div>
				<span class="flex items-center h-8 bg-indigo-200 text-indigo-600 text-sm px-2 rounded">{thing.price}$</span>
			</div>
		</div>
